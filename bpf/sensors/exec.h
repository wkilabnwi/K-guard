...
static __always_inline void handle_suspicious_lineage(struct exec_event *e, struct scratch_buffer *scratch) {
    ...
    if (lin) {
        lin->suspicious_ancestor = 1;
        __builtin_memcpy(lin->ancestor_filename, e->filename, sizeof(lin->ancestor_filename));
    }
}
...
int tp_schedexec(struct sched_process_exec_args *ctx) {
    ...
    long read_res = read_path(filename, PATH_BUF_SIZE, fn, 0, &truncated);
